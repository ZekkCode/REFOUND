...
export default function ReportCard({ item }: ReportCardProps) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200/70 shadow-xs p-5 flex flex-col justify-between space-y-4">
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-[#0B1633] line-clamp-3 leading-relaxed">
            {item.deskripsi_publik}
          </p>
        </div>
      </div>

      <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-zinc-500">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-zinc-700">
            {item.nama_zona || item.id_zona}
          </span>
          <span>&bull;</span>
          <span>{item.waktu_kejadian?.slice(0, 16).replace('T', ' ')}</span>
        </div>
