    ...
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="bg-zinc-50 dark:bg-zinc-800/60 text-xs font-bold uppercase tracking-wider text-zinc-500 border-b border-zinc-200 dark:border-zinc-800">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {daftarLaporan.map((item) => (
              <tr key={item.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/40 transition-colors">
                ...
                <td className="p-4">
                  <span className="font-bold text-xs block text-[#0B1633] dark:text-white">
                    {item.kategori}
                  </span>
                ...
                <td className="p-4 text-right">
                  {onPilihLaporan && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-[#0B1633] hover:bg-[#152754] text-white text-xs font-bold rounded-lg transition-all"
                    >
                      Kelola Custody &rarr;
                    </button>
                  )}
