...
export default function HalamanPencocokanAI() {
  ...
  const [showMatch2, setShowMatch2] = useState(true);
  const [notif, setNotif] = useState<string | null>(null);

  const handleIgnore = (id: number) => {
    if (id === 1) setShowMatch1(false);
    if (id === 2) setShowMatch2(false);
    setNotif('Barang telah ditandai sebagai bukan milik Anda dan disembunyikan.');
    setTimeout(() => {
      setNotif(null);
    }, 4500);
  };
    ...
    <div className="min-h-screen flex bg-[#F9FAFC] text-[#0B1633] font-sans antialiased selection:bg-[#12A99A]/20">
        ...
        <div className="flex-1 py-8 px-6 sm:px-12 max-w-5xl w-full mx-auto space-y-8">
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              {/* Toast Notification */}
              {notif && (
                <div className="bg-[#0B1633] text-white px-5 py-3.5 rounded-2xl text-xs font-bold shadow-lg flex items-center justify-between animate-pulse">
                  ...
                  <button onClick={() => setNotif(null)} className="text-zinc-400 hover:text-white font-black text-[10px] uppercase ml-3">
                    Tutup
                  </button>
                </div>
              )}
                ...
                <div className="bg-white rounded-3xl border border-zinc-150 shadow-[0_10px_35px_rgba(11,22,51,0.02)] overflow-hidden relative">
                  ...
                  <div className="px-6 py-4 bg-zinc-50/50 border-t border-zinc-100 flex flex-col sm:flex-row gap-4 items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex-1 w-full px-6 py-2.5 bg-white border border-[#006F69] hover:bg-[#006F69]/5 text-[#006F69] font-bold text-sm rounded-xl transition-all duration-300"
                    >
                      Bukan Barang Saya
                    </button>
                  </div>
              ...
              {!showMatch1 && !showMatch2 && (
                <div className="text-center py-16 bg-white rounded-3xl border border-zinc-150 p-8 space-y-4">
                  <div className="text-zinc-300 flex justify-center">
                    <svg className="w-16 h-16 text-zinc-200" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                      <circle cx="12" cy="12" r="10" />
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01" />
                    </svg>
                  </div>
                  <h3 className="text-lg font-bold text-[#0B1633]">Tidak ada kandidat kecocokan</h3>
                  <p className="text-zinc-400 text-xs font-semibold max-w-sm mx-auto leading-relaxed">
                    Anda telah mengabaikan semua barang temuan. Kami akan memberi tahu Anda jika AI menemukan kandidat kecocokan baru untuk laporan Anda.
                  </p>
                </div>
              )}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-200 flex items-center justify-around py-2.5 z-50 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        ...
        >
          ...
          <span className="text-[10px] font-black">Kecocokan</span>
        </Link>
        <Link
          href="/dashboard"
          className="flex flex-col items-center space-y-0.5 text-zinc-400 hover:text-[#006F69] transition-colors"
